...
async function buscarJuegos(query) {
  const body = `
    search "${query}";
    fields name,cover.url,summary,genres.name,platforms.name,screenshots.url,involved_companies.developer,involved_companies.company.name,first_release_date,rating;
    limit 10;
  `;
...
// Próximos estrenos: juegos con fecha de lanzamiento futura (paginado)
async function proximosEstrenos(offset = 0, limit = 24) {
  const ahora = Math.floor(Date.now() / 1000);
  const en6meses = ahora + 60 * 60 * 24 * 180;

  const body = `
  fields name,cover.url,summary,genres.name,platforms.name,screenshots.url,first_release_date,hypes;
